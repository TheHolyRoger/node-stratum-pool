...
var pool = module.exports = function pool(options, authorizeFn){
    ...
    function GetFirstJob(finishedCallback){
        ...
        GetBlockTemplate(function(error, result){
            ...
            Object.keys(options.ports).forEach(function(port){
                var checkDiffs = [options.ports[port].diff];
                if (options.ports[port].varDiff) {
                    checkDiffs.push(options.ports[port].varDiff.minDiff);
                    checkDiffs.push(options.ports[port].varDiff.maxDiff);
                }
                checkDiffs.forEach(function(portDiff) {
    ...
    function SetupPeer(){
        ...
        _this.peer.on('connected', function() {
            if (!process.env.forkId || process.env.forkId === '0')
                emitWarningLog('p2p connection successful');
    ...
    function StartStratumServer(finishedCallback){
        ...
        _this.stratumServer.on('started', function(){
            ...
            if (!process.env.forkId || process.env.forkId === '0')
                emitWarningLog('No new blocks for ' + options.jobRebroadcastTimeout + ' seconds - updating transactions & rebroadcasting work');
            ...
            client.on('difficultyChanged', function(diff){
                ...
                if (fullMessage.method != 'mining.extranonce.subscribe') {
    ...
    function SetupBlockPolling(){
        ...
        blockPollingIntervalId = setInterval(function () {
            GetBlockTemplate(function(error, result, foundNewBlock){
                if (foundNewBlock)
                    if (!process.env.forkId || process.env.forkId === '0')
                        emitWarningLog('Block notification via RPC polling');
            });
        }, pollingInterval);
    }
    ...
    **/
    this.processBlockNotify = function(blockHash, sourceTrigger) {
        if (!process.env.forkId || process.env.forkId === '0')
            emitWarningLog('Block notification via ' + sourceTrigger);
    ...
    this.setVarDiff = function(port, varDiffConfig) {
        ...
        _this.varDiff[port].on('newDifficulty', function(client, newDiff) {
            ...
            if (varDiffConfig.mode === 'fast'){
                ...
                var result = client.sendDifficulty(newDiff);
                if (result) {
                    client.emit('difficultyChanged', client.difficulty);
